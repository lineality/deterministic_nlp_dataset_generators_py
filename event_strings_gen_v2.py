...
training, evaluation, and unified cross-validation datasets in JSON Lines (JSONL)
format. It systematically combines event nomenclature and temporal modifiers into
natural language question templates spanning multiple syntactic structures.

Dataset partitioning into training, evaluation, and K-Fold cross-validation splits
is achieved deterministically using salted cryptographic hashing (SHA-256) rather
than pseudo-random permutations, ensuring platform-independent consistency.

CLI Modes Supported:
    1. 'split': Writes separate training and evaluation JSONL files.
    2. 'unified': Writes a single consolidated JSONL file containing complete records
       annotated with deterministic K-Fold cross-validation fold indices.

(Boiler Plate by Gemini)
"""
...
class ConfigurationParameterError(DatasetGenerationError):
    """Raised when invalid configuration parameters, ratios, or modes are provided."""
...
# =====================================================================
# VALIDATION AND DETERMINISTIC ASSIGNMENT FUNCTIONS
# =====================================================================
...
def compute_deterministic_cross_validation_fold(
    ...
    """
    Assign a sample record deterministically to a zero-indexed cross-validation fold.

    Uses a distinct salted SHA-256 namespace ('::cv_fold::') to guarantee statistical
    independence from binary train/test splits.

    Parameters:
        unique_record_content_identifier: Unique string identifier for the record.
        total_cross_validation_folds_count: Total integer count of folds (e.g., 5 or 10).
        partition_salt_string: Cryptographic salt string.

    Returns:
        An integer fold index in the range [0, total_cross_validation_folds_count - 1].

    Raises:
        ConfigurationParameterError: If total_cross_validation_folds_count is less than 2.
    """
    ...
    try:
        ...
        extracted_integer_value: int = int.from_bytes(digest_bytes[:8], byteorder="big", signed=False)
        assigned_fold_index: int = extracted_integer_value % total_cross_validation_folds_count

        return assigned_fold_index

    except Exception as runtime_exception:
        detailed_traceback: str = traceback.format_exc()
        diagnostic_message: str = (
            f"Encountered unexpected failure calculating CV fold for identifier "
            f"'{unique_record_content_identifier}': {runtime_exception}\n{detailed_traceback}"
        )
        raise DatasetGenerationError(diagnostic_message) from runtime_exception
...
def construct_level_one_record_dictionary(
    ...
    Parameters:
        ...
        total_cross_validation_folds_count: Number of folds for CV indexing.
    ...
    assigned_cv_fold_index: int = compute_deterministic_cross_validation_fold(
        unique_record_content_identifier=rendered_query_text,
    ...
    return {
        ...
        "deterministic_cross_validation_fold_index": assigned_cv_fold_index,
    }
...
def generate_all_dataset_records(
    ...
    Parameters:
        ...
        total_cross_validation_folds_count: Cardinality of cross-validation folds.
...
def partition_and_save_split_datasets(
    ...
    """
    Partition dataset records into separate train and test JSONL files.
...
def persist_unified_dataset(
    all_generated_records_sequence: Sequence[dict[str, object]],
    output_unified_jsonl_file_path: Path,
) -> int:
    """
    Persist all records into a single consolidated JSONL dataset file.

    Parameters:
        all_generated_records_sequence: Full sequence of generated records.
        output_unified_jsonl_file_path: Destination path for the unified JSONL file.

    Returns:
        Total integer count of persisted lines.
    """
    return write_records_to_jsonl_file_path(
        record_dictionary_sequence=all_generated_records_sequence,
        target_destination_file_path=output_unified_jsonl_file_path,
    )
...
def execute_deterministic_dataset_generation_pipeline(
    ...
    dataset_organization_mode_string: str = "split",
    total_cross_validation_folds_count: int = 5,
    ...
    """
    Orchestrate dataset generation and dispatch persistence according to organization mode.

    Parameters:
        output_directory_path: Base directory where output JSONL files will be created.
        dataset_organization_mode_string: Either 'split' or 'unified'.
        total_cross_validation_folds_count: Number of folds for CV fold assignments.
        training_split_ratio_floating_point: Ratio allocated to training in 'split' mode.
        partition_salt_string: Cryptographic salt for deterministic hashing.
        candidate_event_names: Sequence of candidate event names.
        candidate_time_terms: Sequence of candidate temporal expressions.
        level_one_templates: Sequence of 1-level templates.
        level_two_modular_templates: Sequence of 2-level modular templates.

    Returns:
        A dictionary containing execution diagnostics, persisted paths, and counts.

    Raises:
        ConfigurationParameterError: If dataset_organization_mode_string is unrecognized.
    """
    normalized_mode_string: str = dataset_organization_mode_string.strip().lower()
    if normalized_mode_string not in ("split", "unified"):
        unrecognized_mode_message: str = (
            f"Unrecognized dataset organization mode '{dataset_organization_mode_string}'. "
            "Supported modes are strictly 'split' or 'unified'."
        )
        raise ConfigurationParameterError(unrecognized_mode_message)

    print("[PIPELINE NOTICE] Starting deterministic dataset generation pipeline...")
    print(f"[PIPELINE NOTICE] Dataset Organization Mode: '{normalized_mode_string}'")
    ...
    print(f"[PIPELINE NOTICE] Partition Salt String: '{partition_salt_string}'")
    ...
    print(f"[PIPELINE NOTICE] Total Inquiries Generated: {len(generated_records)}")

    pipeline_result_dictionary: dict[str, object] = {
        "execution_mode": normalized_mode_string,
        "total_records_generated": len(generated_records),
    }

    if normalized_mode_string == "split":
        train_file_path: Path = output_directory_path / "train_event_time_queries.jsonl"
        test_file_path: Path = output_directory_path / "test_event_time_queries.jsonl"

        train_count, test_count = partition_and_save_split_datasets(
            all_generated_records_sequence=generated_records,
            output_training_jsonl_file_path=train_file_path,
            output_evaluation_jsonl_file_path=test_file_path,
        )

        observed_train_ratio: float = (train_count / len(generated_records)) if generated_records else 0.0
        print(f"[PIPELINE SUCCESS] Persisted Training File: {train_file_path} (Count: {train_count}, Ratio: {observed_train_ratio:.2%})")
        print(f"[PIPELINE SUCCESS] Persisted Test File: {test_file_path} (Count: {test_count}, Ratio: {1.0 - observed_train_ratio:.2%})")

        pipeline_result_dictionary["training_file_path"] = str(train_file_path)
        pipeline_result_dictionary["training_records_count"] = train_count
        pipeline_result_dictionary["evaluation_file_path"] = str(test_file_path)
        pipeline_result_dictionary["evaluation_records_count"] = test_count

    elif normalized_mode_string == "unified":
        unified_file_path: Path = output_directory_path / "unified_complete_dataset.jsonl"

        unified_persisted_count: int = persist_unified_dataset(
            all_generated_records_sequence=generated_records,
            output_unified_jsonl_file_path=unified_file_path,
        )

        print(f"[PIPELINE SUCCESS] Persisted Unified File: {unified_file_path} (Total Count: {unified_persisted_count})")
        print(f"[PIPELINE NOTICE] Annotated Cross-Validation Folds: 0 through {total_cross_validation_folds_count - 1}")

        pipeline_result_dictionary["unified_file_path"] = str(unified_file_path)
        pipeline_result_dictionary["unified_records_count"] = unified_persisted_count
        pipeline_result_dictionary["cross_validation_folds_count"] = total_cross_validation_folds_count

    return pipeline_result_dictionary
...
def construct_command_line_argument_parser() -> argparse.ArgumentParser:
    """
    Construct and configure the command-line interface argument parser.
    ...
    argument_parser: argparse.ArgumentParser = argparse.ArgumentParser(
        description=(
            "Deterministic JSONL generator for temporal event inquiry datasets. "
            "Supports binary split partitioning (train/test) or single unified datasets "
            "with deterministic K-Fold cross-validation annotations."
        ),
    ...
    argument_parser.add_argument(
        ...
        default="split",
        help=(
            "Dataset output organization structure: 'split' outputs separate train and test "
            "JSONL files; 'unified' outputs a single JSONL file containing cross-validation folds."
        ),
    )
    ...
    argument_parser.add_argument(
        "-o",
        "--output-directory",
        dest="output_directory_path_string",
        type=str,
        default="./generated_dataset_output",
        help="Target filesystem directory where output JSONL files will be created.",
    )

    argument_parser.add_argument(
        "-r",
        "--train-split-ratio",
        dest="training_split_ratio_floating_point",
        type=float,
        default=0.80,
        help="Proportion of generated data allocated to training (used in 'split' mode).",
    )
    ...
    argument_parser.add_argument(
        ...
        type=int,
        default=5,
        help="Total number of deterministic cross-validation folds (used in 'unified' mode).",
    )
    ...
    argument_parser.add_argument(
        ...
        type=str,
        default="production_dataset_partition_salt_v2026",
        help="Cryptographic salt string guaranteeing invariant hash-based partitioning.",
    )

    return argument_parser


def main_execution_entrypoint() -> None:
    """
    ...
    try:
        target_output_directory_path: Path = Path(parsed_cli_arguments.output_directory_path_string)

        execute_deterministic_dataset_generation_pipeline(
            output_directory_path=target_output_directory_path,
            dataset_organization_mode_string=parsed_cli_arguments.dataset_organization_mode,
            total_cross_validation_folds_count=parsed_cli_arguments.cross_validation_folds_count,
            training_split_ratio_floating_point=parsed_cli_arguments.training_split_ratio_floating_point,
        ...
        detailed_traceback_string: str = traceback.format_exc()
        sys.stderr.write(
            f"\n[FATAL PIPELINE ERROR] Generation terminated due to an unhandled exception:\n"
            f"{unhandled_execution_exception}\n\n"
